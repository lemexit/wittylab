...
<?php if (!isset($_GET["q"]) || empty($_GET["q"])): ?>
  <div id="showmore" class="hide-callback panel panel-body panel-dark">
    <p>Please note that by using this tool some fields such as title, description and tags will auto-populate itself with their respective vimeo data. You might need to edit each one to fix them as Dailymotion data might contain unwanted characters or wordings.</p>
  </div><!--/#showmore-->     
  <div class="panel panel-default">
    <div class="panel-heading">Dailymotion Mass Importer <a href="" data-target="#showmore" class="toggle btn btn-xs btn-primary pull-right">Help</a></div>     
      <form method="get" action="<?php echo Main::ahref("media/dailymotion")?>" class="form">
        <div class="panel-body">              
          ...
          <div class="row">
            ...
            <div class="col-md-4">
              ...
              <select name="o" id="o">
                <option value="visited">Views</option>
  ...
  <div class="row">
    <div class="col-md-9">
      <div class="panel panel-default">
        <div class="panel-heading">
          Search results for "<?php echo Main::clean($_GET['q'],3,TRUE); ?>" <a href="<?php echo Main::ahref("media/dailymotion")?>" class="btn btn-xs btn-primary pull-right">Back to Importer</a>
        </div>
        <div class="panel-body">
          <div class="row">
              ...
              <tbody>
                <?php 
                  ...
                  foreach ($data["list"] as $key):
                    $id = $key["id"];
                    $val[] = $id;
                    if(!$key["allow_embed"]) continue;
                ?>    
                  <tr id='yt-<?php echo $n?>' class='mediainfo'>
                    <td width='12%'>
                      <a href="" class='modal-trigger' title='<?php echo Main::truncate($key["title"],40)?>' data-content="<iframe style='border:0;width: 100%;height:400px;' src='<?php echo $key["embed_url"] ?>' frameborder='0' allowfullscreen></iframe>"><img src="<?php echo $key["thumbnail_480_url"] ?>" width='100'/><br /></a>
                    </td>
                    <td>
                      <input type="checkbox" class="this-import data-delete-check" name="media[]" value="<?php echo $id ?>">&nbsp;&nbsp;&nbsp;<a href="<?php echo $key["url"] ?>"  target="blank" title='<?php echo Main::truncate($key["title"],100)?>' class='medialink'><?php echo Main::truncate($key["title"],60)?> <small class="pull-right"><?php echo Main::timeago(date("Y-m-d H:i:s",$key["created_time"]))?></small></a>                      
                      ...
                      <div class="row">
                        ...
                        <div class="col-md-6" id="import-data">
                          <div class="btn-group btn-group-sm pull-right">
                            ...
                            <a href="<?php echo Main::ahref("media/import&url=".urlencode($key["url"]),"media/import?url=".urlencode($key["url"])) ?>" class="btn btn-success">Customize</a>
                          </div>
                        </div>
                      </div>
                    </td>
          ...
          <hr>
            <?php $action=(!$this->config["mod_rewrite"])?"action=Dailymotion&":"" ?>
            <?php if(isset($_GET["page"])):?>
              <a href="?<?php echo $action ?>q=<?php echo $q?>&amp;i=<?php echo $i?>&amp;type=<?php echo $type?>&amp;o=<?php echo $o?>&amp;<?php echo $pre_p[0] ?>" class="btn btn-primary">Previous Page
              </a>
            <?php endif;?>
            <a href="?<?php echo $action ?>q=<?php echo $q?>&amp;type=<?php echo $type?>&amp;i=<?php echo $i?>&amp;o=<?php echo $o?>&amp;<?php echo $next_p[0] ?>" class="btn btn-primary">
              Next Page
            </a>  
          <hr>                 
        </div>
    ...
    <div class="col-md-3">
      <div class="panel panel-default">
        ...
        <div class="panel-body">
          <form method="get" action="<?php echo Main::ahref("media/dailymotion") ?>">
            <div class="form-group">
              <label>Keyword</label>
